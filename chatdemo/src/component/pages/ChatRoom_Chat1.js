...
const ChatRoom = ({ roomType }) => {
  ...
  useLayoutEffect(() => {
    ...
    const storedMessages = JSON.parse(localStorage.getItem(`chat1`)) || [];
    setMessages(storedMessages);

    const storedchatAnswered = JSON.parse(localStorage.getItem(`chatAnswered1`)) || false;
  ...
  const handleSendMessage = async () => {
    ...
    const newMessage = {
      type: roomType === "ChatChat1" ? "questioner" : "answerer",
  ...
  return (
    <div className="w-[390px] h-[844px] flex flex-col bg-gradient-to-b from-white to-[#c6d6f8]">
      ...
      <div className="py-4 px-2 flex justify-between items-center">
        <button
          onClick={() =>{
            navigate(roomType === "ChatChat1" ? "/ChatDashboard" : "/ChatDashboard");
        ...
        <h1 className="text-lg font-bold">
          {roomType === "ChatChat1" ? "Suvee Srigeaw" : "Chat Room"}
        </h1>
      </div>
          ...
          <div key={index} className={`flex ${getMessageAlignment(message)} mb-2`}>
            <div
              className={`px-4 py-2 rounded-lg ${
                message.type === (roomType === "ChatChat1" ? "questioner" : "answerer")
      ...
      <div className="p-4 flex items-center bg-white">
        <textarea
          ...
          placeholder={
            roomType === "ChatChat1" ? "message..." : "message..."
          }
        ></textarea>
        <button
